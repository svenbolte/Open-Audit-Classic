...
*/
	...
	Function EndSendingMessage()
	{
		...
		&& !IsSet($has["cc"])
		&& !IsSet($has["bcc"]))
			return($this->OutputError("The following headers were not set: To, Cc or Bcc"));
		if(!IsSet($has["subject"]))
			return($this->OutputError("The Subject header is not set"));
		...
		&& $this->auto_message_id)
		{
			...
			if(count($senders) == 0)
				return('No valid sender addresses were specified'.(strlen($error) ? ': '.$error : ''));
			Reset($senders);
			$sender=Key($senders);
	...
	Function GetPartHeaders(&$headers,$part)
	{
		...
		if(!IsSet($this->parts[$part]["Content-Type"]))
			return($this->OutputError("A part was added without a defined Content-Type"));
	...
	Function GetPartBody(&$body,$part)
	{
		...
		if(!IsSet($this->parts[$part]["Content-Type"]))
			return($this->OutputError("Added a part without a defined Content-Type"));
		$type=$this->Tokenize($full_type=strtolower($this->parts[$part]["Content-Type"]),"/");
		$sub_type=$this->Tokenize("");
		$body="";
		switch($type)
			...
			case "message":
				...
				else
				{
					if(!IsSet($this->parts[$part]["DATA"]))
						return($this->OutputError("A part was added without a body PART"));
...
*/
	...
	Function GetFileDefinition($file, &$definition, $require_name=1)
	{
		...
		{
			if(!IsSet($file["Data"]))
				return($this->OutputError("A file part name was not specified"));
		}
		if(IsSet($file["Name"]))
			$name=$file["Name"];
		if($require_name
...
*/
	Function GetDataURL($file)
	{
		...
		else
		{
			...
			{
				$this->OutputError("A file or data block was not specified");
				return("");
			}
...
*/
	...
	Function OpenMailing(&$mailing,&$mailing_properties)
	{
		...
		|| strlen($mailing_properties["Name"])==0)
			return($this->OutputError("A valid mailing Name was not specified"));
		if(!IsSet($mailing_properties["Return-Path"])
		|| strlen($mailing_properties["Return-Path"])==0)
			return($this->OutputError("A valid mailing Return-Path was not specified"));
		$separator="";
	...
	Function AddMailingRecipient($mailing,&$recipient_properties)
	{
		...
		if(!IsSet($this->mailings[$mailing]))
			return($this->OutputError("A valid mailing was not specified"));
		if(!IsSet($recipient_properties["Address"])
		|| strlen($recipient_properties["Address"])==0)
			return($this->OutputError("A valid mailing recipient Address was not specified"));
